...
for _ in $(seq 1 60); do
  ...
  if [ "$MATCHED" = "True" ]; then
    ...
    BASTION_X=$(python3 -c "import json,sys;print(json.load(sys.stdin).get('bastionX',0))" <<<"$RESP")
    BASTION_Z=$(python3 -c "import json,sys;print(json.load(sys.stdin).get('bastionZ',0))" <<<"$RESP")
...
if [ -n "${MATCH_ID:-}" ]; then
  TRACE=$(npx tsx "$(dirname "$0")/synthTrace.ts" "$FINISH" 10 \
      "${STRUCT_X:-0}" "${STRUCT_Z:-0}" "${BASTION_X:-0}" "${BASTION_Z:-0}" 2>/dev/null)
  if [ -n "$TRACE" ]; then
